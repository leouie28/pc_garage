            ...
            <template v-slot:top>
              ...
              >
                ...
                <v-dialog v-model="dialogDelete" max-width="390px">
                  <v-card>
                      <v-card-title class="text-h5">Are you sure you want to delete this variation?</v-card-title>
                      <v-card-actions>
                      <v-spacer></v-spacer>
...
<script>
  export default {
    ...
    methods: {
      ...
      editDialog(item){
        this.dialog = true
        this.formTitle = 'Edit Variation'
        this.payload = JSON.parse(JSON.stringify(item))
      },

      addDialog(){
        this.formTitle = 'Add Variation'
        this.dialog = true
      },
      
      editItem (id) {
        axios.put('/admin/variation/update'+id).then(({data}) => {
          console.log('Success');
          this.initialize();
        })
      },
      ...
      deleteItemConfirm (id) {
        axios.delete('/admin/variation/destroy/'+this.payload.id, this.payload).then(({data}) => {
          console.log('Success');
          this.initialize();
      ...
      save () { 
        ...
        if(this.payload.id) {
          axios.put('/admin/variation/update/'+this.payload.id, this.payload).then(({data}) => {
            this.initialize()
            this.close()
          }).catch(error => {
          ...
          axios.post('/admin/variation/create', this.payload).then(({data}) => {
            this.initialize()
            this.close()
          }).catch(error => {
            if(error.response.data.errors.name) {
